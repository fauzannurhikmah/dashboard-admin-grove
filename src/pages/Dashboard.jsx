...
export default function Dashboard() {
  ...
  const isFormPage =
    [
      ...
      "/dashboard/income-statements/create",
      "/dashboard/balance-sheets/create",
  ...
  const getCurrentFormKey = () => {
    ...
    if (location.pathname.includes("/income-statements"))
  ...
  const currentRecordId =
    location.pathname.match(/\/dashboard\/[^/]+\/([^/]+)\/edit$/)?.[1] || null;

  // Struktur menu berdasarkan section/kategori
  const menuSections = [
    {
      title: "Masterdata",
      items: [
    ...
    },
    {
      ...
      items: [
        {
          path: "/dashboard/income-statements",
          label: "Income Statement",
          icon: TrendingUp,
        },
        {
          path: "/dashboard/balance-sheets",
          label: "Balance Sheet",
          icon: Wallet,
        },
  ...
  return (
    <div className="min-h-screen bg-[#09090b] text-zinc-200 font-sans antialiased text-sm tracking-tight selection:bg-emerald-500/30 selection:text-emerald-400">
      <div className="flex h-screen overflow-hidden">
        ...
        <aside
          className={`${sidebarOpen ? "w-64" : "w-16"} bg-[#09090b] border-r border-zinc-900 flex flex-col transition-all duration-300 ease-[cubic-bezier(0.4,0,0.2,1)]`}
        >
          <div className="h-16 flex items-center px-4 border-b border-zinc-900 justify-between">
            <div className="flex items-center gap-2.5 overflow-hidden">
              ...
              {sidebarOpen && (
                <span className="font-semibold text-zinc-100 text-[14px] tracking-tight">
                  Grove Admin
                </span>
              )}
            </div>
          ...
          <nav className="flex-1 py-4 px-2 space-y-6 overflow-y-auto">
            {menuSections.map((section) => (
              <div key={section.title}>
                {/* Judul Section (Hanya muncul saat sidebar terbuka) */}
                {sidebarOpen && (
                  <h3 className="px-3 mb-2 text-[10px] font-bold uppercase tracking-wider text-zinc-500">
                    {section.title}
                  </h3>
                )}
                ...
                <div className="space-y-1">
                  {section.items.map((item) => (
                    <button
                      ...
                      className={`w-full flex items-center rounded-lg transition-all duration-150 group relative ${
                        ...
                        location.pathname.startsWith(item.path)
                          ? "bg-zinc-900 text-emerald-400 font-medium border border-zinc-800"
                    ...
                    >
                      ...
                      {location.pathname.startsWith(item.path) && (
                        <div className="absolute left-0 top-2 bottom-2 w-[2px] bg-emerald-500 rounded-r-sm" />
                      )}
                    </button>
                  ))}
                </div>
          ...
          <div className="p-2 border-t border-zinc-900 mt-auto">
            <button
              ...
              className={`w-full flex items-center rounded-lg text-zinc-500 hover:bg-red-950/20 hover:text-red-400 transition-all duration-150 ${
                sidebarOpen
                  ? "px-3 py-2 gap-3 justify-start"
                  : "p-2 justify-center"
              }`}
            ...
            <div className="flex items-center gap-4">
              {!sidebarOpen && (
                <button
                  ...
                  className="p-1.5 rounded-lg bg-zinc-900 border border-zinc-800 text-zinc-400 hover:text-zinc-100 transition-all"
                >
          ...
          <div className="flex-1 flex overflow-hidden">
            <main className="flex-1 overflow-y-auto p-6 bg-[#09090b]">
              <Routes>
                ...
                />
                <Route
                  path="/income-statements/create"
                  element={<IncomeStatementForm />}
                />
                <Route
                  path="/income-statements/:id/edit"
                  element={<IncomeStatementForm />}
                />
                ...
                <Route
                  path="/balance-sheets/create"
                  element={<BalanceSheetForm />}
                />
                <Route
                  path="/balance-sheets/:id/edit"
                  element={<BalanceSheetForm />}
                />
